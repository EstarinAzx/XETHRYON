...
export function buildMemoryLines(
  ...
  const howToSave = [
        "## How to save knowledge (YOUR BRAIN)",
        "",
        "Your knowledge base IS your brain. It persists between sessions. Organize it like neural pathways — not a junk drawer.",
        "",
        "**NEVER write to MEMORY.md or flat files. Always write to `knowledge/<category>/`.**",
        "",
        "### Routing rules — classify BEFORE you save",
        "",
        "Read each piece of information and route it to the FIRST matching category:",
        "",
        "1. **Is it about a PERSON?** → `knowledge/people/` (name, role, preferences, expertise)",
        "   - `people/marcus.md` — Lead backend dev, designed the order flow",
        "",
        "2. **Is it about a TOOL, library, or service?** → `knowledge/tools/` (config, version, why we use it)",
        "   - `tools/prisma.md` — ORM for PostgreSQL, considering migration to Drizzle",
        "",
        "3. **Is it a HOW-TO or step-by-step?** → `knowledge/qa/` (direct answers, runbooks)",
        "   - `qa/local-dev-setup.md` — docker-compose up, npm run dev, port 3000",
        "",
        "4. **Is it a DECISION with tradeoffs?** → `knowledge/decisions/` (why X over Y, ADRs)",
        "   - `decisions/websockets-over-sse.md` — bidirectional needed for driver location updates",
        "",
        "5. **Is it a PATTERN or technique?** → `knowledge/techniques/` (algorithms, conventions, approaches)",
        "   - `techniques/repository-pattern.md` — never call Prisma directly from route handlers",
        "",
        "6. **Does it CONNECT 2+ existing concepts?** → `knowledge/connections/` (flows, integrations, cross-cutting)",
        "   - `connections/order-payment-flow.md` — links [[api-versioning]] + [[integer-money-storage]] + [[redis-caching]]",
        "",
        "7. **None of the above?** → `knowledge/concepts/` (last resort — standalone facts, preferences)",
        "   - `concepts/api-naming-convention.md` — snake_case for JSON, camelCase for TypeScript",
        "",
        "8. **Need a new category?** Create it. `knowledge/endpoints/`, `knowledge/schemas/`, `knowledge/bugs/` — whatever fits.",
        "",
        "**IMPORTANT: concepts/ is the CATCH-ALL, not the default. Route to a specific folder first.**",
        "",
        "### Article format",
        "",
